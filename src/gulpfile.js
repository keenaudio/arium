...
$.createProjectFile = function(options) {
  ...
  if (fs.existsSync(outPath)) {
    $.util.log("File already exists: ", outPath);
    return;
  }
...
gulp.task('nfo', function() {
  $.util.log("NFO in directory: ", $.args.dir);
  return gulp.src('**/*.mp4', {
      cwd: $.args.dir,
      read: false
    })
...
gulp.task('ingest', function() {
  ...
  return gulp.src($.config.get("ingest.input"), {
      buffer: false
    })
    .pipe(moggFilter)
    ...
    .pipe($.ingest($.config.getRaw("ingest.output.als"), {
      folder: true
    }))
...
gulp.task('oggdec', function() {
  return gulp.src($.config.get("oggdec.input"), {
      buffer: false
    })
...
gulp.task('meta', function() {
  return gulp.src($.config.get("meta.input"), {
      buffer: false
    })
...
gulp.task('explode', function() {
  return gulp.src($.config.get("explode.input"), {
      buffer: false
    })
    //.pipe($.debug({ verbose: true }))
    .pipe($.args.force ? $.through() : $.hasChanged($.config.getRaw("explode.output"), {
      tracknum: 1
...
gulp.task('als2json', function() {
  return gulp.src($.config.get("als2json.input"), {
      buffer: false
    })
...
gulp.task('als2daw', function() {
  return gulp.src($.config.get("als2daw.input"), {
      buffer: false
    })
...
gulp.task('test2', function(cb) {
  ...
  gulp.src($.config.get('paths.templates') + '/Untitled.als.json')
    ...
    .on('end', function() {
      ...
      gulp.src($.config.get('paths.folders') + '/**/*.wav', {
        read: false
      })
        .pipe($.debug({
          verbose: true
